...
d3.chart("hierarchy").extend("partition.arc", {
  ...
  initialize : function() {
    ...
    // chart.diameter(chart.options.diameter || Math.min(chart.options.width, chart.options.height));
    chart.diameter(chart.options.diameter || window.innerHeight * .9);
      ...
      events: {
        "enter": function() {
          ...
          this.attr("class", function(d) {
            if (d["depth"] > 0 && d[chart.options.name] == d["parent"][chart.options.name]) return "selfNamed";
            else if (d["depth"] == 0) return "root";
          });

          this.attr("data-diameter", function(d) {
            if (d["depth"] == 0)
              return chart.options.diameter;
          });

          // this.attr("data-id", function(d) { return d["i"] });
          this.attr("id", function(d) { return d["i"] });
          this.attr("data-name", function(d) {
          ...
          $.each(this[0], function(key, val) {
            ...
            chart.layers.base.append("text")
              .attr("id", id)
              // .attr("transform", "translate(0, -100)")
              // .on("click", function(event) { chart.trigger("click:label", val); })
              .append("textPath")
                .attr("href", "#txt"+ id)
                .text(val.attr("data-name"))
                .attr("startOffset", "5px")
                .attr("text-anchor", "start")
                ;

            chart.layers.base.append("path")
              .attr("d", function() {
                var path = val.attr("d");
                return path.split("L")[0];
              })
              .attr("class", 'txtPath')
              .attr("id", "txt"+ id)
              ;
