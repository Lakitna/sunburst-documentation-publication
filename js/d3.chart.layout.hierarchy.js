 ...
 *
 * Edited by Lakitna to use sunburst diagram as navigation element
 */
...
d3.chart("hierarchy").extend("partition.arc", {
  ...
  initialize : function() {
    ...
    // chart.diameter(chart.options.diameter || Math.min(chart.options.width, chart.options.height));
    chart.diameter(chart.options.diameter || window.innerHeight * .925);


    chart.d3.x   = d3.scale.linear().range([0, rads * Math.PI]);
    chart.d3.y   = d3.scale.sqrt().range([0, chart.options.diameter / 2]);
    chart.d3.arc = d3.svg.arc()
      .startAngle (function(d) { return Math.max(0, Math.min(rads * Math.PI, chart.d3.x(d.x))); })
      .endAngle   (function(d) { return Math.max(0, Math.min(rads * Math.PI, chart.d3.x(d.x + d.dx))); })
    ...
    chart.layers.base
      .attr("transform", "translate(" + (chart.options.height / 4) * (rads-1) + "," + chart.options.height / 2 + ")");
      ...
      events: {
        "enter": function() {
          ...
          this.append("path")
            ...
            .classed("shape", true)
            .classed("selfNamed", function(d) { if (d["depth"] > 0 && d[chart.options.name] == "_"+d["parent"][chart.options.name]) return true; })
            .classed("root", function(d) { if (d["depth"] == 0) return true; })
            .classed("leaf", function(d) { return d.isLeaf; })
            .classed("fixedWidth", function(d) { return d[chart.options.name].match(/^-/); })
            .attr("id", function(d) { return d["i"] })
            .attr("data-name", function(d) {
              return d[chart.options.name].replace(/^-/, '');
            })
            ;
          ...
          this.append("text")
            .attr("id", function(d) { return d["i"] })
            .append("textPath")
              .attr("xlink:href", function(d) { return "#txt"+d["i"] })
              .text(function(d) { return d[chart.options.name].replace(/^-/, ''); })
          ...
          this.append("path")
            .attr("d", function(d) {
              var e = d3.select("path[id='"+d.i+"']");
              return e.attr("d").split("L")[0];
            })
            .attr("class", 'txtPath')
            .attr("id", function(d) { return "txt"+d["i"] })
            ;
  ...
  collapsible: function() {
    ...
    chart.layers.base.on("merge", function() {
      var g = this;
      var path = d3.selectAll("g path.shape");

      chart.off("click:path").on("click:path", function(d) {
        if (d.isLeaf) {
          path.transition()
            .duration(chart.options.duration)
            .attrTween("d", arcTween(d.parent))
            .each("start", function(d) { labelHide(d); })
            .each("end", function(d) { labelUpdate(d); })
            ;
        }
        else {
          path.transition()
        ...
        function labelHide(d) {
          var id = d.i;
          d3.select( $("text[id^="+id+"] textPath")[0] )
            // .attr("style", "opacity:0")
            .attr("style", "opacity:0; display:none")
            ;
        }
        function labelUpdate(d) {
          var id = d.i;
          var p = d3.select( $("path[id^="+id+"]")[0] ).attr("d");

          d3.select( $("path[id^=txt"+id+"]")[0] )
            .attr("d", function(d, i) {
              return p.split("L")[0];
            })
            ;
          ...
          d3.select( $("text[id^="+id+"] textPath")[0] )
            ...
            .attr("style", "opacity:1; display:block")
            ;
        }
    ...
    function arcTween(d) {
      var xd = d3.interpolate(chart.d3.x.domain(), [d.x, d.x + d.dx]),
          ...
          yr = d3.interpolate(chart.d3.y.range(),  [d.y ? (chart.options.diameter / 7) : 0, chart.options.diameter / 2]);
          // console.log(chart.options.diameter);

      return function(d, i) {
        return i ? function(t) {
                  var ret = chart.d3.arc( d );
                  // ret = d3.interpolate(chart.d3.y.domain(), [d.y, 1]);
                  return ret;
                }
                 : function(t) {
